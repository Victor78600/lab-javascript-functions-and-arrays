...
function findLongestWord(myArray) {
  ...
  for (let i = 0; i < myArray.length; i++) {
    let numbChar = myArray[i].length;
    if (numbChar > longestWord.length) {
...
function sum(myArray) {
  ...
  const avg = sumNumbers(myArray) / myArray.length;
  return avg;
}
...
function doesWordExist(myArray, a) {
  if (!myArray.length) {
    return null;
  }
  for (let i = 0; i < myArray.length; i++) {
...
function howManyTimes(myArray, a) {
  if (!myArray.length) {
    return 0;
  }
  let count = 0;
